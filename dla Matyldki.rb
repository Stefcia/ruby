...
def rozmowy(ilośćZakręceń, zaczynajacy, co, rozmowca)
	...
	rozmówki.each do |metadane|
		...
		if i.odd?
			...
			when 30..45 || 67
				wypowiedź =:"#{[:"Mam w domu makaron z sosem #{[:"czosnkowym", :"jabłkowym (z ryżem i fasolką)"].sample}", :"Ale fajne bum z tego brudnego P-Jota! Ha!"].sample}"
			else
				if i > 105
					sleep 6
					...
					if i < 105
						puts :"ok, ok."
						sleep 6
						wypowiedź = :"#{[:"O! to przecież BMW, a nie Alfa Romeo!", :"E, tam! Przestarzały model.", :"Mam pięknego SiangYanga. A ty?", :"Moja siostra ma Alfa Romeo Stelwio.", :"O, widzisz Pontiac! A za nim Alfa Romeo i Suzuki!",:"W moich 10 stajniach jest 10 mysich dziur. W każdej miałam na początku 10 mysich par, które rodzą rocznie 20 młodych(5 dziewcząt i 5 chłopaków). Mam je 3,5 lat.", :"W każdej 1 stajni są 2 pary kotów. Koty te w jeden rok rodzą(każde pary rodzą!) 8 kociąt. Ma koty od 2 lat. Ile mam kotów?",:"Lubisz wchodzić do obory?", :"Ja mam 10 stajni, a na początku miałam w każdej 5 par koni, które rodzą rocznie 6 młodych.", :"W każdej z moich stajnii jest 6 szczurzych dziur. W każdej z tych dziur miałam na początku 2 pary, które rocznie rodzą 2 młodych(1 szczurzycę i 1 szczura). Każda kocia para zjada rocznie 8 szczurów. Mam szczurze dziury od 3 lat.", :"Mam też kozy, ale o tym pomówimy później.", :"super twoja nowa koszulka... Ja mam podobną, tylko że niebieskawo-czerwono-fioletowo-zielono"].sample}"
					end
				end
			end
...
end	return :"dziękujemy za twoje zakupy:#{zamówienie}."
end
sleep 1
kontoM = {
	:hasło => :"Matyldka",
